...
public class Main2 {
    ...
    public static void main(String[] args) throws NumberFormatException, IOException {
        ...
        List<boolean[][]> winnersMarks = new ArrayList<>();
        List<Integer> winnersDraws = new ArrayList<Integer>();
        ...
        while (drawIt.hasNext()) {
            ...
            for (int i = 0; i < boards.size(); i++) {
                ...
                if (winners.contains(currBoard)) {
                    continue;
                }
                mark(currBoard, mark, draw);

                if (isWinner(mark)) {
                    winners.add(currBoard);
                    winnersMarks.add(mark);
                    winnersDraws.add(draw);
                }
            }

        }
        Integer[][] lastBoard = winners.get(winners.size() - 1);
        boolean[][] lastBoardMark = winnersMarks.get(winners.size() - 1);
        int lastDraw = winnersDraws.get(winners.size() - 1);

        int result = calculateResult(lastBoard, lastBoardMark);
        System.out.println(result * lastDraw);
